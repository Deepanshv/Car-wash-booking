...
import { Box, TextField, Button, FormControl, InputLabel, Select, MenuItem, Typography, Paper, SelectChangeEvent, Stack, Grid, Alert, CircularProgress, Collapse } from '@mui/material';
...
const BookingFormPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof AxiosError ? (err.response?.data?.message || 'Failed to save booking.') : 'An unexpected error occurred.');
    } finally {
  ...
  return (
    <Paper elevation={0} sx={{ p: { xs: 2, sm: 4 } }}>
        ...
        <Collapse in={!!error} sx={{ mb: 2 }}><Alert severity="error" onClose={() => setError(null)}>{error}</Alert></Collapse>
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 2 }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mt: 3 }}>
                <Button type="submit" variant="contained" color="primary" size="large" fullWidth disabled={isSubmitting} startIcon={isSubmitting ? <CircularProgress size={20} color="inherit" /> : null}>
