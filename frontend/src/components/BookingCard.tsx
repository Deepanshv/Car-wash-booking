...
import { Card, CardContent, CardActions, Typography, IconButton, Divider, Chip, Box, Menu, MenuItem, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions as MuiDialogActions, Button, CircularProgress } from '@mui/material';
  ...
  return (
    <>
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        ...
        <IconButton onClick={() => setShowConfirm(true)} aria-label="delete">
          <DeleteIcon />
        </IconButton>
        <IconButton
          aria-label="more"
    ...
    >
      ...
      <MuiDialogActions sx={{ p: 2, pt: 0 }}>
        <Button onClick={() => setShowConfirm(false)} disabled={isDeleting} variant="outlined">Cancel</Button>
        <Button onClick={handleDelete} color="error" variant="contained" disabled={isDeleting} startIcon={isDeleting ? <CircularProgress size={20} color="inherit" /> : <DeleteIcon />} autoFocus>
