  ...
  return (
    ...
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '12px', transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': { transform: 'scale(1.02)', boxShadow: 6, } }}>
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        ...
        <IconButton onClick={() => setDeleteOpen(true)} aria-label="delete booking" color="error">
          <DeleteIcon />
        </IconButton>
        <IconButton
          aria-label="more"
    ...
    <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
      ...
      <DialogContent>
        <DialogContentText>
          Delete booking for <strong>{booking.customerName}</strong>? This cannot be undone.
        </DialogContentText>
      </DialogContent>
