    ...
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '12px', transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': { transform: 'scale(1.02)', boxShadow: 6, } }}>
      <CardContent sx={{ flexGrow: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <Typography variant="h5" component="div">{booking.customerName}</Typography>
            <Chip label={booking.status} color={statusColors[booking.status]} size="small" />
        </Box>
        <Typography sx={{ mb: 1.5 }} color="text.secondary">{booking.carDetails.make} {booking.carDetails.model}</Typography>
        <Divider sx={{ my: 1 }} />
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
            <Typography variant="subtitle2" color="text.secondary" sx={{ mr: 1 }}>Service:</Typography>
            <Typography variant="body1" color="text.primary">{booking.serviceType}</Typography>
        </Box>
      </CardContent>
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        <IconButton component={Link} to={`/edit/${booking._id}`} aria-label="edit"><EditIcon /></IconButton>
        <IconButton onClick={() => setOpenConfirm(true)} aria-label="delete" disabled={isDeleting}><DeleteIcon /></IconButton>
        <IconButton aria-label="more" id="long-button" aria-controls={open ? 'long-menu' : undefined} aria-expanded={open ? 'true' : undefined} aria-haspopup="true" onClick={handleClick}><MoreVertIcon /></IconButton>
      ...
      <Dialog open={openConfirm} onClose={() => !isDeleting && setOpenConfirm(false)} aria-labelledby="delete-dialog-title" aria-describedby="delete-dialog-description" PaperProps={{ sx: { backgroundColor: 'background.paper', backgroundImage: 'none' } }}>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setOpenConfirm(false)} disabled={isDeleting} variant="outlined">Cancel</Button>
          <Button onClick={handleConfirmDelete} color="error" variant="contained" disabled={isDeleting} autoFocus startIcon={isDeleting && <CircularProgress size={20} color="inherit" />}>
