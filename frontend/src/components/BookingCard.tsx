...
const BookingCard: React.FC<BookingCardProps> = ({ booking, onDelete, onUpdateStatus }) => {
  ...
  return (
    ...
    <Dialog open={openDeleteDialog} onClose={isDeleting ? undefined : () => setOpenDeleteDialog(false)}>
        ...
        <DialogActions>
            ...
            <Button onClick={handleDeleteConfirm} color="error" variant="contained" disabled={isDeleting} startIcon={isDeleting && <CircularProgress size={20} color="inherit" />}>{isDeleting ? 'Deleting...' : 'Delete'}</Button>
        </DialogActions>
    </Dialog>
    </>
  );
