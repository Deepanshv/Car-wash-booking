  ...
  return (
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '12px', transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': { transform: 'scale(1.02)', boxShadow: 6, } }}>
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        ...
        <IconButton onClick={() => setConfirmOpen(true)} aria-label="delete">
          <DeleteIcon />
        </IconButton>
        <IconButton
          aria-label="more"
      ...
      <Dialog open={confirmOpen} onClose={isDeleting ? undefined : () => setConfirmOpen(false)}>
        <DialogTitle>Confirm Deletion</DialogTitle>
        <DialogContent>
          <DialogContentText>Delete booking for <strong>{booking.customerName}</strong>?</DialogContentText>
        </DialogContent>
        <DialogActions>
