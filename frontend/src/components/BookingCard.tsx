  ...
  return (
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '12px', transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': { transform: 'scale(1.02)', boxShadow: 6, } }}>
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        ...
        <IconButton onClick={() => setIsDialogOpen(true)} aria-label="delete" color="error">
          <DeleteIcon />
        </IconButton>
        <IconButton
          aria-label="more"
      ...
      <Dialog
        open={isDialogOpen}
        onClose={isDeleting ? undefined : () => setIsDialogOpen(false)}
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setIsDialogOpen(false)} disabled={isDeleting}>
            Cancel
          </Button>
          <Button
