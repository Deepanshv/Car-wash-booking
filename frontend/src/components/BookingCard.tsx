...
const BookingCard: React.FC<BookingCardProps> = ({ booking, onDelete, onUpdateStatus }) => {
  ...
  return (
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '12px', transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': { transform: 'scale(1.02)', boxShadow: 6, } }}>
      ...
      <Dialog open={showConfirm} onClose={isDeleting ? undefined : () => setShowConfirm(false)} PaperProps={{ sx: { borderRadius: '12px' } }}>
        <DialogTitle sx={{ fontWeight: 600 }}>Delete Booking?</DialogTitle>
        <DialogContent>
          <DialogContentText>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button onClick={handleDeleteConfirm} color="error" variant="contained" disabled={isDeleting} startIcon={isDeleting ? <CircularProgress size={20} color="inherit" /> : undefined}>
