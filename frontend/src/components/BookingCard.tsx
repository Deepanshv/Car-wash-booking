  ...
  return (
    <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '12px', transition: 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out', '&:hover': { transform: 'scale(1.02)', boxShadow: 6, } }}>
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        ...
        <IconButton onClick={() => setOpenDeleteDialog(true)} aria-label="delete">
          <DeleteIcon />
        </IconButton>
        <IconButton
          aria-label="more"
      ...
      <Dialog open={openDeleteDialog} onClose={() => !isDeleting && setOpenDeleteDialog(false)} aria-labelledby="del-title" PaperProps={{ sx: { backgroundColor: 'background.paper', backgroundImage: 'none' } }}>
        <DialogTitle id="del-title">Confirm Deletion</DialogTitle>
